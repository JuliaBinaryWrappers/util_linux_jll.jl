...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import util_linux_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("util_linux_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "util_linux_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"util_linux", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "util_linux_jll", "src", "util_linux_jll.jl"))
        @info("util_linux_ll dev'ed out to /depot/dev/util_linux_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("a762b42e-dc87-5958-a639-9c9eec9c0153"))
platforms = [Pkg.Artifacts.unpack_platform(e, "util_linux", artifacts_toml) for e in artifacts["util_linux"]]
...
if best_platform === nothing
    @debug("Unable to load util_linux; unsupported platform $(triplet(platform_key_abi()))")
else
